...
SELECT 
CASE
WHEN resale_price LIKE '%Lakh%' THEN REGEXP_SUBSTR(resale_price, '([0-9]*[.])?[0-9]+ ?([A-Z][a-z]*)?')
WHEN resale_price LIKE '%Crore%' THEN REGEXP_SUBSTR(resale_price, '([0-9]*[.])?[0-9]+ ?([A-Z][a-z]*)?')
WHEN resale_price LIKE '%,%' THEN REPLACE(REGEXP_SUBSTR(resale_price, '([0-9]*[.])?[0-9]+([,][0-9]+)? ?([A-Z][a-z]*)?'), ',', '')
END AS fixed_price
FROM car_resale
where registered_year = 2007
;
...
-- Fill the column
UPDATE car_resale
SET price_fixed = CASE
	WHEN resale_price LIKE '%Lakh%' THEN REGEXP_SUBSTR(resale_price, '([0-9]*[.])?[0-9]+ ?([A-Z][a-z]*)?')
	WHEN resale_price LIKE '%Crore%' THEN REGEXP_SUBSTR(resale_price, '([0-9]*[.])?[0-9]+ ?([A-Z][a-z]*)?')
	WHEN resale_price LIKE '%,%' THEN REPLACE(REGEXP_SUBSTR(resale_price, '([0-9]*[.])?[0-9]+([,][0-9]+)? ?([A-Z][a-z]*)?'), ',', '')
	END;

----------------------------------------------------------- Get prices in numbers only updated -----------------------------------------------------------
